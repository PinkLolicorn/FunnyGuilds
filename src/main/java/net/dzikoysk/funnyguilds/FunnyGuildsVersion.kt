...
class FunnyGuildsVersion(private val funnyGuilds: FunnyGuilds) {
    val fullVersion: String
    val mainVersion: String
    fun isNewAvailable(sender: CommandSender, force: Boolean) {
        if (!funnyGuilds.pluginConfiguration.updateInfo && !force) {
            return
        }
        if (!sender.hasPermission("funnyguilds.admin")) {
            return
        }
        funnyGuilds.server.scheduler.runTaskAsynchronously(funnyGuilds, Runnable {
            val latest = IOUtils.getContent(FunnyGuildsVersion.Companion.VERSION_FILE_URL) ?: return@runTaskAsynchronously
            if (latest.contains("Warning:")) {
                FunnyGuilds.Companion.getPluginLogger().warning(latest)
                return@runTaskAsynchronously
            }
            val currentNightlyHash = getCurrentNightlyHash(latest)
            if (StringUtils.isNotBlank(currentNightlyHash)) {
                if (!funnyGuilds.pluginConfiguration.updateNightlyInfo) {
                    return@runTaskAsynchronously
                }
                try {
                    val ghResponse = IOUtils.getContent(FunnyGuildsVersion.Companion.GH_COMMITS_URL)
                    val ghCommits: JsonArray = FunnyGuildsVersion.Companion.GSON.fromJson<JsonArray>(ghResponse, JsonArray::class.java)
                    if (ghCommits.size() == 0) {
                        return@runTaskAsynchronously
                    }
                    val latestCommit = ghCommits[0].asJsonObject
                    val commitHash = latestCommit["sha"].asString.substring(0, 7)
                    if (commitHash != currentNightlyHash) {
                        printNewVersionAvailable(sender, "$latest-$commitHash", true)
                    }
                } catch (th: Throwable) {
                    FunnyGuilds.Companion.getPluginLogger().update("Could not retrieve latest nightly version!")
                    FunnyGuilds.Companion.getPluginLogger().update(Throwables.getStackTraceAsString(th))
                }
                return@runTaskAsynchronously
            }
            if (!latest.equals(fullVersion, ignoreCase = true)) {
                printNewVersionAvailable(sender, latest, false)
            }
        })
    }

    private fun printNewVersionAvailable(sender: CommandSender, latest: String, isNightly: Boolean) {
        sender.sendMessage("")
        sender.sendMessage(ChatColor.DARK_GRAY.toString() + "-----------------------------------")
        sender.sendMessage(ChatColor.GRAY.toString() + "Dostepna jest nowa wersja " + ChatColor.AQUA + "FunnyGuilds" + (if (isNightly) " Nightly" else "") + ChatColor.GRAY + '!')
        sender.sendMessage(ChatColor.GRAY.toString() + "Obecna: " + ChatColor.AQUA + fullVersion)
        sender.sendMessage(ChatColor.GRAY.toString() + "Najnowsza: " + ChatColor.AQUA + latest)
        sender.sendMessage(ChatColor.GRAY.toString() + "GitHub: " + ChatColor.AQUA + FunnyGuildsVersion.Companion.GITHUB_URL)
        sender.sendMessage(ChatColor.GRAY.toString() + "Discord: " + ChatColor.AQUA + FunnyGuildsVersion.Companion.DISCORD_URL)
        sender.sendMessage(ChatColor.DARK_GRAY.toString() + "-----------------------------------")
        sender.sendMessage("")
    }

    private fun getCurrentNightlyHash(latest: String): String {
        val remainder = StringUtils.replace(fullVersion, latest, "").trim { it <= ' ' }

        // hyphen + short commit hash
        return if (remainder.length != 8 || !remainder.startsWith("-")) {
            ""
        } else StringUtils.replace(remainder, "-", "")
    }

    companion object {
        private val GSON = Gson()
        private const val GH_COMMITS_URL = "https://api.github.com/repos/FunnyGuilds/FunnyGuilds/commits"
        private const val VERSION_FILE_URL = "https://funnyguilds.dzikoysk.net/latest.info"
        private const val GITHUB_URL = "https://github.com/funnyguilds/funnyguilds"
        private const val DISCORD_URL = "https://discord.com/invite/CYvyq3u"
    }

    init {
        val version = funnyGuilds.description.version
        fullVersion = version
