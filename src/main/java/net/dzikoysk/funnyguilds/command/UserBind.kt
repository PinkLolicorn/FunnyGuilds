...
@FunnyComponent
internal class UserBind : Bind {
    override fun accept(injectorResources: Resources) {
        injectorResources.on(User::class.java).assignHandler { property: Property?, annotation: Any?, args: Array<Any?> -> fetchUser(CommandUtils.getContext(*args)) }
    }

    fun fetchUser(context: Context): User {
        val commandSender = context.commandSender
        check(commandSender is OfflinePlayer) { "Cannot use user bind in non-player command" }
        return User.Companion.get(commandSender as OfflinePlayer)
    }
}