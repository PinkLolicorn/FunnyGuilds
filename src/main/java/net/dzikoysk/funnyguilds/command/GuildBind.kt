...
@FunnyComponent
internal class GuildBind : Bind {
    override fun accept(injectorResources: Resources) {
        injectorResources.on(Guild::class.java).assignHandler { property: Property?, annotation: Any?, args: Array<Any?> ->
            val user: User = GuildBind.Companion.USER_BIND.fetchUser(CommandUtils.getContext(*args))
            if (!user.hasGuild()) {
                throw ValidationException(FunnyGuilds.Companion.getInstance().getMessageConfiguration().generalHasNoGuild)
            }
            user.guild
        }
    }
    ...
    companion object {
        private val USER_BIND = UserBind()
    }
}