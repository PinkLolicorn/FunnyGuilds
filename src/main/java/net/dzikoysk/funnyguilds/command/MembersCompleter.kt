...
@FunnyComponent
internal class MembersCompleter : Completer {
    override fun apply(context: Context, prefix: String, limit: Int): List<String> {
        return Option.`when`<User>(context.commandSender is Player, User.Companion.get(context.commandSender as OfflinePlayer))
            .filter { obj: User -> obj.hasGuild() }
            .map { obj: User -> obj.guild }
            .map { guild: Guild? ->
                CommandUtils.collectCompletions(
                    guild!!.members, prefix, limit, { initialCapacity: Int? -> ArrayList(initialCapacity!!) }) { obj: User? -> obj!!.name }
            }
            .orElseGet(emptyList())
    }
