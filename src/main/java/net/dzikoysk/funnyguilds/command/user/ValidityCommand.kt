...
class ValidityCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, @CanManage user: User?, guild: Guild) {
        if (config.validityWhen != 0L) {
            val validity = guild.validity
            val delta = validity - System.currentTimeMillis()
            DefaultValidation.`when`(delta > config.validityWhen, messages.validityWhen.replace("{TIME}", TimeUtils.getDurationBreakdown(delta - config.validityWhen)))
        }
        val requiredItems = config.validityItems
        if (!ItemUtils.playerHasEnoughItems(player, requiredItems)) {
            return
        }
        if (!SimpleEventHandler.handle(GuildExtendValidityEvent(EventCause.USER, user, guild, config.validityTime))) {
            return
        }
        ...
        player.sendMessage(messages.validityDone.replace("{DATE}", config.dateFormat!!.format(Date(validity))))
    }
}