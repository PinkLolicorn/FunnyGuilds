...
class ConfirmCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, @IsOwner user: User, guild: Guild) {
        ...
        DefaultValidation.`when`(!ConfirmationList.contains(user.uuid), messages.deleteToConfirm)
        ConfirmationList.remove(user.uuid)
        if (!SimpleEventHandler.handle(GuildDeleteEvent(EventCause.USER, user, guild))) {
            return
        }
