...
@FunnyComponent
class FunnyGuildsCommand(private val funnyGuilds: FunnyGuilds) {
    @FunnyCommand(name = "\${user.funnyguilds.name}", description = "\${user.funnyguilds.description}", aliases = ["\${user.funnyguilds.aliases}"], acceptsExceeded = true)
    fun execute(sender: CommandSender, args: Array<String>) {
        val parameter = if (args.size > 0) args[0].toLowerCase() else ""
        when (parameter) {
            "reload", "rl" -> reload(sender)
            "check", "update" -> funnyGuilds.version.isNewAvailable(sender, true)
            "save-all" -> saveAll(sender)
            "funnybin" -> post(sender, args)
            "help" -> {
                sender.sendMessage(ChatColor.AQUA.toString() + "FunnyGuilds Help:")
                sender.sendMessage(ChatColor.GRAY.toString() + "/funnyguilds (reload|rl) - przeladuj plugin")
                sender.sendMessage(ChatColor.GRAY.toString() + "/funnyguilds (update|check) - sprawdz dostepnosc aktualizacji")
                sender.sendMessage(ChatColor.GRAY.toString() + "/funnyguilds save-all - zapisz wszystko")
                sender.sendMessage(ChatColor.GRAY.toString() + "/funnyguilds funnybin - zapisz konfigurację online (~ usprawnia pomoc na https://github.com/FunnyGuilds/FunnyGuilds/issues)")
            }
            else -> sender.sendMessage(ChatColor.GRAY.toString() + "FunnyGuilds " + ChatColor.AQUA + funnyGuilds.version.fullVersion + ChatColor.GRAY + " by " + ChatColor.AQUA + "FunnyGuilds Team")
        }
    }

    private fun reload(sender: CommandSender) {
        DefaultValidation.`when`(!sender.hasPermission("funnyguilds.reload"), FunnyGuilds.Companion.getInstance().getMessageConfiguration().permission)
        sender.sendMessage(ChatColor.GRAY.toString() + "Przeladowywanie...")
        FunnyGuilds.Companion.getInstance().getConcurrencyManager().postRequests(ReloadRequest(sender))
    }

    private fun saveAll(sender: CommandSender) {
        DefaultValidation.`when`(!sender.hasPermission("funnyguilds.admin"), FunnyGuilds.Companion.getInstance().getMessageConfiguration().permission)
        sender.sendMessage(ChatColor.GRAY.toString() + "Zapisywanie...")
        val currentTime = System.currentTimeMillis()
        val dataModel: DataModel = FunnyGuilds.Companion.getInstance().getDataModel()
        try {
            dataModel.save(false)
            FunnyGuilds.Companion.getInstance().getInvitationPersistenceHandler().saveInvitations()
        } catch (e: Exception) {
            FunnyGuilds.Companion.getPluginLogger().error("An error occurred while saving plugin data!", e)
            return
        }
        sender.sendMessage(ChatColor.GRAY.toString() + "Zapisano (" + ChatColor.AQUA + (System.currentTimeMillis() - currentTime) / 1000.0f + "s" + ChatColor.GRAY + ")!")
    }

    private fun post(sender: CommandSender, args: Array<String>) {
        DefaultValidation.`when`(!sender.hasPermission("funnyguilds.admin"), FunnyGuilds.Companion.getInstance().getMessageConfiguration().permission)
        val request: Optional<FunnybinRequest> = FunnybinRequest.Companion.of(sender, args)
        if (request.isPresent) {
            FunnyGuilds.Companion.getInstance().getConcurrencyManager().postRequests(request.get())
            return
        }
        sender.sendMessage(ChatColor.RED.toString() + "Uzycie: ")
        sender.sendMessage(ChatColor.RED.toString() + "/fg funnybin - domyslnie wysyla FunnyGuilds/config.yml i logs/latest.log")
        sender.sendMessage(ChatColor.RED.toString() + "/fg funnybin config - wysyla FunnyGuilds/config.yml")
        sender.sendMessage(ChatColor.RED.toString() + "/fg funnybin log - wysyla logs/latest.log")
        sender.sendMessage(ChatColor.RED.toString() + "/fg funnybin custom <path> - wysyla dowolny plik z folderu serwera na funnybina")
        sender.sendMessage(ChatColor.RED.toString() + "/fg funnybin bundle <file1> <fileN...> - wysyla dowolne pliki z folderu serwera na funnybina")
    }
}