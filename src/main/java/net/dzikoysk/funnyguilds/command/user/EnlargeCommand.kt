...
class EnlargeCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, @CanManage user: User, guild: Guild) {
        DefaultValidation.`when`(!config.regionsEnabled, messages.regionsDisabled)
        val region = guild.region
        DefaultValidation.`when`(region == null, messages.regionsDisabled)
        var enlarge = region.enlarge
        DefaultValidation.`when`(enlarge > config.enlargeItems!!.size - 1, messages.enlargeMaxSize)
        val need = config.enlargeItems!![enlarge]
        DefaultValidation.`when`(!player.inventory.containsAtLeast(need, need!!.amount), messages.enlargeItem.replace("{ITEM}", need.amount.toString() + " " + need.type.toString().toLowerCase()))
        DefaultValidation.`when`(RegionUtils.isNear(region.center), messages.enlargeIsNear)
        if (!SimpleEventHandler.handle(GuildEnlargeEvent(EventCause.USER, user, user.guild))) {
            return
        }
        player.inventory.removeItem(need)
        region.enlarge = ++enlarge
        region!!.size = region.size + config.enlargeSize
        guild.broadcast(
            messages.enlargeDone
                .replace("{SIZE}", Integer.toString(region.size))
                .replace("{LEVEL}", Integer.toString(region.enlarge))
        )
    }
}