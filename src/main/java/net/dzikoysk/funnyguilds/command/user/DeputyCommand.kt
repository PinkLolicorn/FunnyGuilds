...
class DeputyCommand {
    ...
    fun execute(messages: MessageConfiguration, player: Player, @IsOwner owner: User, guild: Guild, args: Array<String?>) {
        DefaultValidation.`when`(args.size < 1, messages.generalNoNickGiven)
        val deputyUser = UserValidation.requireUserByName(args[0])
        DefaultValidation.`when`(owner == deputyUser, messages.deputyMustBeDifferent)
        DefaultValidation.`when`(!guild.members.contains(deputyUser), messages.generalIsNotMember)
        if (!SimpleEventHandler.handle(GuildMemberDeputyEvent(EventCause.USER, owner, guild, deputyUser))) {
            return
        }
        val deputyPlayer = Option.of(deputyUser.player)
        if (deputyUser.isDeputy) {
            guild.removeDeputy(deputyUser)
            player.sendMessage(messages.deputyRemove)
            deputyPlayer.peek { value: Player -> value.sendMessage(messages.deputyMember) }
            return
        }
        guild.addDeputy(deputyUser)
        player.sendMessage(messages.deputySet)
        deputyPlayer.peek { value: Player -> value.sendMessage(messages.deputyOwner) }
    }
}