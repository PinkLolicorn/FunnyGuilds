...
class InviteCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, @CanManage user: User?, guild: Guild, args: Array<String?>) {
        ...
        DefaultValidation.`when`(guild.members.size >= config.maxMembersInGuild, messages.inviteAmount.replace("{AMOUNT}", Integer.toString(config.maxMembersInGuild)))
        val invitedUser = UserValidation.requireUserByName(args[0])
        val invitedPlayer = invitedUser.player
        if (InvitationList.hasInvitationFrom(invitedUser, guild)) {
            if (!SimpleEventHandler.handle(GuildMemberRevokeInviteEvent(EventCause.USER, user, guild, invitedUser))) {
                return
            }
            InvitationList.expireInvitation(guild, invitedUser)
            player.sendMessage(messages.inviteCancelled)
            DefaultValidation.`when`(invitedPlayer != null, messages.inviteCancelledToInvited.replace("{OWNER}", player.name).replace("{GUILD}", guild.name!!).replace("{TAG}", guild.tag))
            return
        }
        DefaultValidation.`when`(invitedPlayer == null, messages.invitePlayerExists)
        DefaultValidation.`when`(invitedUser.hasGuild(), messages.generalUserHasGuild)
        if (!SimpleEventHandler.handle(GuildMemberInviteEvent(EventCause.USER, user, guild, invitedUser))) {
            return
        }
        InvitationList.createInvitation(guild, invitedPlayer)
        player.sendMessage(messages.inviteToOwner.replace("{PLAYER}", invitedPlayer!!.name))
        invitedPlayer.sendMessage(messages.inviteToInvited.replace("{OWNER}", player.name).replace("{GUILD}", guild.name!!).replace("{TAG}", guild.tag))
    }
}