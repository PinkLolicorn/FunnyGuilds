...
class EscapeCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, user: User) {
        DefaultValidation.`when`(!config.regionsEnabled, messages.regionsDisabled)
        DefaultValidation.`when`(!config.escapeEnable || !config.baseEnable, messages.escapeDisabled)
        DefaultValidation.`when`(user.cache.teleportation != null, messages.escapeInProgress)
        val playerLocation = player.location
        val region = RegionUtils.getAt(playerLocation)
        DefaultValidation.whenNull(region, messages.escapeNoNeedToRun)
        val time: Int = FunnyGuilds.Companion.getInstance().getPluginConfiguration().escapeDelay
        if (!user.hasGuild()) {
            DefaultValidation.`when`(!config.escapeSpawn, messages.escapeNoUserGuild)
            scheduleTeleportation(player, user, player.world.spawnLocation, time) {}
            return
        }
        val guild = user.guild
        DefaultValidation.`when`(guild == region!!.guild, messages.escapeOnYourRegion)
        if (time >= 1) {
            player.sendMessage(messages.escapeStartedUser.replace("{TIME}", Integer.toString(time)))
            val msg = messages.escapeStartedOpponents.replace("{TIME}", Integer.toString(time)).replace("{PLAYER}", player.name)
                .replace("{X}", Integer.toString(playerLocation.blockX)).replace("{Y}", Integer.toString(playerLocation.blockY))
                .replace("{Z}", Integer.toString(playerLocation.blockZ))
            for (member in region.guild.onlineMembers) {
                member.player.sendMessage(msg)
            }
        }
        scheduleTeleportation(player, user, guild!!.home, time) {
            for (member in region.guild.onlineMembers) {
                member.player.sendMessage(messages.escapeSuccessfulOpponents.replace("{PLAYER}", player.name))
            }
        }
    }

    private fun scheduleTeleportation(player: Player, user: User, destination: Location?, time: Int, onSuccess: Runnable) {
