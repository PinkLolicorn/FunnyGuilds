...
class DeleteCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player?, @IsOwner user: User, guild: Guild) {
        DefaultValidation.`when`(config.guildDeleteCancelIfSomeoneIsOnRegion && guild.isSomeoneInRegion, messages.deleteSomeoneIsNear)
        ConfirmationList.add(user.uuid)
        DefaultValidation.`when`(config.commands.confirm.enabled, messages.deleteConfirm)
        DeleteCommand.Companion.CONFIRM_EXECUTOR.execute(config, messages, player, user, guild)
    }

    companion object {
        private val CONFIRM_EXECUTOR = ConfirmCommand()
    }
}