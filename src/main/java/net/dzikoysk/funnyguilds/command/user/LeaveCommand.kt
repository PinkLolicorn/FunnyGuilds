...
class LeaveCommand {
    ...
    fun execute(messages: MessageConfiguration, player: Player, @IsMember user: User, guild: Guild) {
        DefaultValidation.`when`(user.isOwner, messages.leaveIsOwner)
        if (!SimpleEventHandler.handle(GuildMemberLeaveEvent(EventCause.USER, user, guild, user))) {
            return
        }
        guild.removeMember(user)
        user.removeGuild()
        val concurrencyManager: ConcurrencyManager = FunnyGuilds.Companion.getInstance().getConcurrencyManager()
        concurrencyManager.postRequests(PrefixGlobalRemovePlayerRequest(user.name), PrefixGlobalUpdatePlayer(player))
