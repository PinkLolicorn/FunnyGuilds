...
class RankResetCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, user: User) {
        val requiredItems = config.rankResetItems
        if (!ItemUtils.playerHasEnoughItems(player, requiredItems)) {
            return
        }
        val lastRank = user.rank.points
        user.rank.points = config.rankStart
        player.inventory.removeItem(*ItemUtils.toArray(requiredItems))
        var resetMessage = messages.rankResetMessage
        resetMessage = StringUtils.replace(resetMessage, "{LAST-RANK}", lastRank.toString())
        resetMessage = StringUtils.replace(resetMessage, "{CURRENT-RANK}", user.rank.points.toString())
