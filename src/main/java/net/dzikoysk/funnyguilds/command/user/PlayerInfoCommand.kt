...
class PlayerInfoCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        DefaultValidation.`when`(args.size == 0 && sender !is Player, messages.playerOnly)
        val name = if (args.size == 0) sender.name else args[0]!!
        val user = UserUtils.get(name, config.playerLookupIgnorecase)
        DefaultValidation.`when`(user == null, messages.generalNotPlayedBefore)
        sendInfoMessage(messages.playerInfoList, user, sender)
    }

    fun sendInfoMessage(baseMessage: List<String?>?, infoUser: User?, messageTarget: CommandSender) {
        ...
        val rank = infoUser.getRank()
        for (messageLine in baseMessage!!) {
            if (infoUser!!.hasGuild()) {
                messageLine = StringUtils.replace(messageLine, "{GUILD}", infoUser.guild!!.name)
                messageLine = StringUtils.replace(messageLine, "{TAG}", infoUser.guild.tag)
            } else {
            ...
            messageLine = StringUtils.replace(messageLine, "{PLAYER}", infoUser.name)
            ...
            messageLine = StringUtils.replace(messageLine, "{RANK}", Integer.toString(rank.position))
