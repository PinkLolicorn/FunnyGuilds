...
class LeaderCommand {
    ...
    fun execute(messages: MessageConfiguration, player: Player, @IsOwner owner: User, guild: Guild, args: Array<String?>) {
        ...
        val leaderUser = UserValidation.requireUserByName(args[0])
        DefaultValidation.`when`(owner == leaderUser, messages.leaderMustBeDifferent)
        DefaultValidation.`when`(!guild.members.contains(leaderUser), messages.generalIsNotMember)
        if (!SimpleEventHandler.handle(GuildMemberLeaderEvent(EventCause.USER, owner, guild, leaderUser))) {
            return
        }
        val leaderPlayer = leaderUser.player
        guild.owner = leaderUser
