...
class SetBaseCommand {
    ...
    fun execute(config: PluginConfiguration, messages: MessageConfiguration, player: Player, @CanManage user: User?, guild: Guild) {
        ...
        val region = RegionUtils.get(guild.name)
        val location = player.location
        DefaultValidation.`when`(!region!!.isIn(location), messages.setbaseOutside)
        if (!SimpleEventHandler.handle(GuildBaseChangeEvent(EventCause.USER, user, guild, location))) {
            return
        }
        guild.home = location
        if (guild.home!!.block.getRelative(BlockFace.DOWN).type == Material.AIR) {
            for (y in guild.home!!.blockY downTo 1) {
                guild.home!!.y = y.toDouble()
                if (guild.home!!.block.type != Material.AIR) {
                    break
                }
            }
        }
