...
class PointsCommand {
    ...
    fun execute(messages: MessageConfiguration, config: PluginConfiguration, sender: CommandSender, args: Array<String>) {
        ...
        DefaultValidation.`when`(args.size < 2, messages.adminNoPointsGiven)
        val points: Int
        points = try {
            args[1].toInt()
        ...
        val change = points - userRank!!.points
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(PointsChangeEvent(AdminUtils.getCause(admin), userRank, admin, change))) {
            return
        }
        user.rank.points = points
        RankManager.Companion.getInstance().update(user)
        var message = messages.adminPointsChanged.replace("{PLAYER}", user.name!!)
        message = message.replace("{POINTS-FORMAT}", IntegerRange.Companion.inRangeToString<String?>(points, config.pointsFormat))
