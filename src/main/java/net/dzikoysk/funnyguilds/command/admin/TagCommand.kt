...
class TagCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String>) {
        DefaultValidation.`when`(args.size < 2, messages.generalNoTagGiven)
        ...
        if (!SimpleEventHandler.handle(GuildPreTagChangeEvent(AdminUtils.getCause(admin), admin, guild, oldTag, tag))) {
            return
        }
        guild.tag = tag
        sender.sendMessage(
            messages.adminTagChanged
                .replace("{OLD_TAG}", oldTag!!)
                .replace("{TAG}", guild.tag)
        )
        SimpleEventHandler.handle(GuildTagChangeEvent(AdminUtils.getCause(admin), admin, guild, oldTag, tag))
    }
}