...
class UnbanCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        DefaultValidation.`when`(!guild.isBanned, messages.adminGuildNotBanned)
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(GuildUnbanEvent(AdminUtils.getCause(admin), admin, guild))) {
            return
        }
        BanUtils.unban(guild)
        val formatter = Formatter()
            .register("{GUILD}", guild.name)
            .register("{TAG}", guild.name)
