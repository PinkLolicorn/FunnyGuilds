...
class ProtectionCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        DefaultValidation.`when`(args.size < 3, messages.adminNoAdditionalProtectionDateGiven)
        val guild = GuildValidation.requireGuildByTag(args[0])
        val protectionDateAsString = StringUtils.join(args, ' ', 1, 3)
        val protectionDate: Date
        protectionDate = try {
            ProtectionCommand.Companion.PROTECTION_DATE_FORMAT.parse(protectionDateAsString)
        } catch (e: ParseException) {
            sender.sendMessage(messages.adminInvalidAdditionalProtectionDate)
            return
        }
        guild.setAdditionalProtection(protectionDate.time)
    ...
    companion object {
        private val PROTECTION_DATE_FORMAT = SimpleDateFormat("yyyy/MM/dd HH:mm:ss")
    }
}