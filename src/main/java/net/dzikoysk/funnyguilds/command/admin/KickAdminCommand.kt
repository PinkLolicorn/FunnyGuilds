...
class KickAdminCommand {
    @FunnyCommand(name = "\${admin.kick.name}", permission = "funnyguilds.admin", acceptsExceeded = true)
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        DefaultValidation.`when`(!user.hasGuild(), messages.generalPlayerHasNoGuild)
        DefaultValidation.`when`(user.isOwner, messages.adminGuildOwner)
        val guild = user.guild
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(GuildMemberKickEvent(AdminUtils.getCause(admin), admin, guild, user))) {
            return
        }
        val concurrencyManager: ConcurrencyManager = FunnyGuilds.Companion.getInstance().getConcurrencyManager()
        concurrencyManager.postRequests(PrefixGlobalRemovePlayerRequest(user.name))
