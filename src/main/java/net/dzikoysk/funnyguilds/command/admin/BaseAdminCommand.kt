...
class BaseAdminCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        val userToTeleport = UserValidation.requireUserByName(args[0])
        DefaultValidation.`when`(!userToTeleport.isOnline, messages.generalNotOnline)
        DefaultValidation.`when`(!userToTeleport.hasGuild(), messages.generalPlayerHasNoGuild)
        val guildHome = userToTeleport.guild!!.home
        DefaultValidation.whenNull(guildHome, messages.adminGuildHasNoHome)
        val formatter = Formatter()
            .register("{ADMIN}", sender.name)
            .register("{PLAYER}", userToTeleport.name)
        userToTeleport.player.teleport(guildHome)
