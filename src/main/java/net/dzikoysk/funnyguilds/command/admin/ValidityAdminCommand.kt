...
class ValidityAdminCommand {
    ...
    fun execute(messages: MessageConfiguration, config: PluginConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        DefaultValidation.`when`(args.size < 2, messages.adminNoValidityTimeGiven)
        val guild = GuildValidation.requireGuildByTag(args[0])
        DefaultValidation.`when`(guild.isBanned, messages.adminGuildBanned)
        val time = TimeUtils.parseTime(args[1])
        if (time < 1) {
            sender.sendMessage(messages.adminTimeError)
            return
        }
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(GuildExtendValidityEvent(AdminUtils.getCause(admin), admin, guild, time))) {
            return
        }
        var validity = guild.validity
        if (validity == 0L) {
            validity = System.currentTimeMillis()
        }
        validity += time
        guild.validity = validity
        val date = config.dateFormat!!.format(Date(validity))
        sender.sendMessage(messages.adminNewValidity.replace("{GUILD}", guild.name!!).replace("{VALIDITY}", date))
    }
}