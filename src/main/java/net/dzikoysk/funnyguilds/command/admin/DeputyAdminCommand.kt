...
class DeputyAdminCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        val userToMove = UserValidation.requireUserByName(args[1])
        DefaultValidation.`when`(!guild.members.contains(userToMove), messages.adminUserNotMemberOf)
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(GuildMemberDeputyEvent(AdminUtils.getCause(admin), admin, guild, userToMove))) {
            return
        }
        val formatter = Formatter().register("{PLAYER}", userToMove.name)
        if (userToMove.isDeputy) {
            ...
            val message = formatter.format(messages.deputyNoLongerMembers)
            for (member in guild.onlineMembers) {
        ...
        val message = formatter.format(messages.deputyMembers)
        for (member in guild.onlineMembers) {
            member.player.sendMessage(message)
        }
    }
