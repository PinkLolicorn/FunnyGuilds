...
class AddCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender?, args: Array<String?>) {
        DefaultValidation.`when`(args.size < 1, messages.generalNoTagGiven)
        DefaultValidation.`when`(!GuildUtils.tagExists(args[0]), messages.generalNoGuildFound)
        DefaultValidation.`when`(args.size < 2, messages.generalNoNickGiven)
        val userToAdd = UserValidation.requireUserByName(args[1])
        DefaultValidation.`when`(userToAdd.hasGuild(), messages.generalUserHasGuild)
        val guild = GuildValidation.requireGuildByTag(args[0])
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(GuildMemberJoinEvent(AdminUtils.getCause(admin), admin, guild, userToAdd))) {
            return
        }
        guild.addMember(userToAdd)
        userToAdd.guild = guild
        FunnyGuilds.Companion.getInstance().getConcurrencyManager().postRequests(PrefixGlobalAddPlayerRequest(userToAdd.name))
        val formatter = Formatter()
            .register("{GUILD}", guild.name)
            .register("{TAG}", guild.tag)
            .register("{PLAYER}", userToAdd.name)
        userToAdd.sendMessage(formatter.format(messages.joinToMember))
        guild.owner!!.sendMessage(formatter.format(messages.joinToOwner))
