...
class KillsCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String>) {
        ...
        DefaultValidation.`when`(args.size < 2, messages.adminNoKillsGiven)
        val kills: Int
        kills = try {
            args[1].toInt()
        ...
        val change = kills - userRank!!.deaths
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(KillsChangeEvent(AdminUtils.getCause(admin), userRank, admin, change))) {
            return
        }
        user.rank.kills = kills
        sender.sendMessage(messages.adminKillsChanged.replace("{PLAYER}", user.name!!).replace("{KILLS}", Integer.toString(kills)))
    }
}