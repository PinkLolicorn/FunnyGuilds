package net.dzikoysk.funnyguilds.command.adminimport
...
class MoveCommand {
    ...
    fun execute(messages: MessageConfiguration, config: PluginConfiguration, player: Player, args: Array<String?>) {
        DefaultValidation.`when`(!config.regionsEnabled, messages.regionsDisabled)
        DefaultValidation.`when`(args.size < 1, messages.generalNoTagGiven)
        val guild = GuildValidation.requireGuildByTag(args[0])
        val location = player.location
        if (config.createCenterY != 0) {
            location.y = config.createCenterY.toDouble()
        }
        var distance = config.regionSize + config.createDistance
        if (config.enlargeItems != null) {
            distance = config.enlargeItems!!.size * config.enlargeSize + distance
        }
        DefaultValidation.`when`(
            distance > LocationUtils.flatDistance(player.world.spawnLocation, location),
            messages.createSpawn.replace("{DISTANCE}", Integer.toString(distance))
        )
        DefaultValidation.`when`(RegionUtils.isNear(location), messages.createIsNear)
        val admin = AdminUtils.getAdminUser(player)
        if (!SimpleEventHandler.handle(GuildMoveEvent(AdminUtils.getCause(admin), admin, guild, location))) {
            return
        }
        var region = guild.region
        if (region == null) {
            region = Region(guild, location, config.regionSize)
        } else {
            if (config.createEntityType != null) {
                ...
                val block = region.center.block.getRelative(BlockFace.DOWN)
        ...
        if (config.createCenterSphere) {
            val sphere = SpaceUtils.sphere(location, 3, 3, false, true, 0)
            for (locationInSphere in sphere!!) {
        ...
        player.sendMessage(messages.adminGuildRelocated.replace("{GUILD}", guild.name!!).replace("{REGION}", region.name!!))
    }
}