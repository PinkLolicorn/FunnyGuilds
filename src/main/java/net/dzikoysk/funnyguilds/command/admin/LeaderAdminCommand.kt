...
class LeaderAdminCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        val user = UserValidation.requireUserByName(args[1])
        DefaultValidation.`when`(!guild.members.contains(user), messages.adminUserNotMemberOf)
        DefaultValidation.`when`(guild.owner == user, messages.adminAlreadyLeader)
        val admin = AdminUtils.getAdminUser(sender)
        if (!SimpleEventHandler.handle(GuildMemberLeaderEvent(AdminUtils.getCause(admin), admin, guild, user))) {
            return
        }
        guild.owner = user
        sender.sendMessage(messages.leaderSet)
        user.sendMessage(messages.leaderOwner)
        val message = messages.leaderMembers.replace("{PLAYER}", user.name!!)
