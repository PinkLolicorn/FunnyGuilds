...
class LivesCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String>) {
        ...
        DefaultValidation.`when`(args.size < 2, messages.adminNoLivesGiven)
        ...
        if (!SimpleEventHandler.handle(GuildLivesChangeEvent(AdminUtils.getCause(admin), admin, guild, lives))) {
            return
        }
        guild.lives = lives
        sender.sendMessage(messages.adminLivesChanged.replace("{GUILD}", guild.tag).replace("{LIVES}", Integer.toString(lives)))
    }
}