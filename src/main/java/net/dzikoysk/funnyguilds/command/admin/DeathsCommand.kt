...
class DeathsCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String>) {
        ...
        DefaultValidation.`when`(args.size < 2, messages.adminNoDeathsGiven)
        val deaths = Option.attempt(NumberFormatException::class.java) { args[1].toInt() }
            .orThrow<RuntimeException> { throw ValidationException(messages.adminErrorInNumber.replace("{ERROR}", args[1])) }
        val admin = AdminUtils.getAdminUser(sender)
        val user = UserValidation.requireUserByName(args[0])
        val userRank = user.rank
        val change = deaths - userRank!!.deaths
        if (!SimpleEventHandler.handle(DeathsChangeEvent(AdminUtils.getCause(admin), userRank, admin, change))) {
            return
        }
        userRank.deaths = deaths
        sender.sendMessage(messages.adminDeathsChanged.replace("{PLAYER}", user.name!!).replace("{DEATHS}", Integer.toString(deaths)))
    }
}