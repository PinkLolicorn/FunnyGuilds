...
class NameCommand {
    ...
    fun execute(messages: MessageConfiguration, config: PluginConfiguration, sender: CommandSender, args: Array<String>) {
        DefaultValidation.`when`(args.size < 1, messages.generalNoTagGiven)
        DefaultValidation.`when`(args.size < 2, messages.adminNoNewNameGiven)
        val guild = GuildValidation.requireGuildByTag(args[0])
        DefaultValidation.`when`(GuildUtils.nameExists(args[1]), messages.createNameExists)
        val admin = AdminUtils.getAdminUser(sender)
        val oldName = guild.name
        if (!SimpleEventHandler.handle(GuildPreRenameEvent(AdminUtils.getCause(admin), admin, guild, oldName, args[1]))) {
            return
        }
        if (config.regionsEnabled) {
            val region = guild.region
            if (FunnyGuilds.Companion.getInstance().getDataModel() is FlatDataModel) {
                val dataModel: FlatDataModel = FunnyGuilds.Companion.getInstance().getDataModel() as FlatDataModel
        ...
        guild.name = args[1]
        sender.sendMessage(messages.adminNameChanged.replace("{GUILD}", guild.name!!))
        SimpleEventHandler.handle(GuildRenameEvent(AdminUtils.getCause(admin), admin, guild, oldName, args[1]))
    }
}