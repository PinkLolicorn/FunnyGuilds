...
class BanCommand {
    ...
    fun execute(messages: MessageConfiguration, sender: CommandSender, args: Array<String?>) {
        ...
        DefaultValidation.`when`(args.size < 2, messages.adminNoBanTimeGiven)
        DefaultValidation.`when`(args.size < 3, messages.adminNoReasonGiven)
        ...
        DefaultValidation.`when`(time < 1, messages.adminTimeError)
        ...
        if (!SimpleEventHandler.handle(GuildBanEvent(AdminUtils.getCause(admin), admin, guild, time, reason))) {
            return
        }
