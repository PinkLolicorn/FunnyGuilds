...
internal object AdminUtils {
    fun getAdminUser(sender: CommandSender): User? {
        return if (sender is Player) User.Companion.get(sender.getName()) else null
    }

    fun getCause(admin: User?): EventCause {
        return if (admin == null) EventCause.CONSOLE else EventCause.ADMIN
    }
}