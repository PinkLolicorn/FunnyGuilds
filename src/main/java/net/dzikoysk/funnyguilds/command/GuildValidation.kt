...
object GuildValidation {
    fun requireGuildByTag(tag: String?): Guild {
        return GuildUtils.getByTag(tag)
            ?: throw ValidationException(StringUtils.replace(FunnyGuilds.Companion.getInstance().getMessageConfiguration().generalGuildNotExists, "{TAG}", tag))
    }
}