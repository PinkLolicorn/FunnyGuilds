...
@FunnyComponent
internal class ManageValidator : Validator<CanManage?, User, ValidationException?> {
    @Throws(ValidationException::class)
    override fun validate(context: Context, annotation: CanManage?, property: Property, user: User): Boolean {
        ManageValidator.Companion.MEMBER_VALIDATOR.isMember(user)
        if (!user.canManage()) {
            throw ValidationException(FunnyGuilds.Companion.getInstance().getMessageConfiguration().generalIsNotOwner)
        }
        return true
    }

    override fun getAnnotation(): Class<CanManage?>? {
        return CanManage::class.java
    }

    override fun getType(): Class<User>? {
        return User::class.java
    }

    companion object {
        private val MEMBER_VALIDATOR = MemberValidator()
    }
}