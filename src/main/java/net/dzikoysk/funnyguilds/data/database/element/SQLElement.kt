package net.dzikoysk.funnyguilds.data.database.elementimport
...
class SQLElement(val key: String, val sQLType: SQLType, private val size: Int, val isNotNull: Boolean) {
    val keyGraveAccent: String
        get() = "`$key`"
    val keyValuesAssignment: String
        get() = "$keyGraveAccent=VALUES($keyGraveAccent)"

    fun getType(): String {
        return if (size != -1) sQLType.getName() + "(" + size + ")" else sQLType.getName()
    }
}