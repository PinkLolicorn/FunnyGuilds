package net.dzikoysk.funnyguilds.dataimport
...
interface DataModel {
    ...
    fun load()
    fun save(ignoreNotChanged: Boolean)
    fun validateLoadedData() {
        if (!FunnyGuilds.Companion.getInstance().getPluginConfiguration().regionsEnabled) {
            return
        }
        var guilds = GuildUtils.getNames(GuildUtils.getGuilds())
        var regions = RegionUtils.getNames(RegionUtils.getRegions())
        var repaired = 0
        for (guild in GuildUtils.getGuilds()) {
            if (guild!!.name != null && regions!!.contains(guild.name)) {
        ...
        for (region in RegionUtils.getRegions()) {
            if (region!!.name != null && guilds.contains(region!!.name)) {
        ...
        if (repaired > 0) {
            FunnyGuilds.Companion.getPluginLogger().warning("Repaired conflicts: $repaired")
        }
    }

    companion object {
        fun create(funnyGuilds: FunnyGuilds?, dataModel: PluginConfiguration.DataModel): DataModel? {
            return if (dataModel == PluginConfiguration.DataModel.MYSQL) {
                SQLDataModel()
            } else FlatDataModel(funnyGuilds)
        }
    }
}