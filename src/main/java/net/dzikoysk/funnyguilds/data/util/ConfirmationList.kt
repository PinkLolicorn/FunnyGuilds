package net.dzikoysk.funnyguilds.data.utilimport
...
object ConfirmationList {
    private val CONFIRMATION_LIST: List<UUID> = ArrayList()
    fun add(uuid: UUID?) {
        if (!ConfirmationList.CONFIRMATION_LIST.contains(uuid)) {
            ConfirmationList.CONFIRMATION_LIST.add(uuid)
        }
    }

    fun remove(uuid: UUID?) {
        ConfirmationList.CONFIRMATION_LIST.remove(uuid)
    }

    operator fun contains(uuid: UUID?): Boolean {
        return ConfirmationList.CONFIRMATION_LIST.contains(uuid)
    }
}