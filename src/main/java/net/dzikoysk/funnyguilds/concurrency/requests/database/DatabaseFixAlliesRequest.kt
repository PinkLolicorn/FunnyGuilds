...
class DatabaseFixAlliesRequest : DefaultConcurrencyRequest() {
    ...
    private fun fixAllies(guild: Guild?) {
        for (ally in guild!!.allies!!) {
            fixAlly(guild, ally)
        }
    }

    private fun fixAlly(guild: Guild?, ally: Guild?) {
        if (!ally!!.allies!!.contains(guild)) {
            ally.addAlly(guild)
        }
    }
}