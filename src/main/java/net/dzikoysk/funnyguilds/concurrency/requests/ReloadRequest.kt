...
class ReloadRequest(private val sender: CommandSender) : DefaultConcurrencyRequest() {
    private val startTime: Long
    override fun execute() {
        val funnyGuilds: FunnyGuilds = FunnyGuilds.Companion.getInstance()
        funnyGuilds.reloadPluginConfiguration()
        funnyGuilds.reloadMessageConfiguration()
        funnyGuilds.dataPersistenceHandler.reloadHandler()
        funnyGuilds.dynamicListenerManager.reloadAll()
        if (FunnyGuilds.Companion.getInstance().getPluginConfiguration().playerListEnable) {
            val config: PluginConfiguration = FunnyGuilds.Companion.getInstance().getPluginConfiguration()
            AbstractTablist.Companion.wipeCache()
        ...
        val diff = String.format("%.2f", (endTime - startTime) / 1000.0)
        sender.sendMessage(ChatColor.AQUA.toString() + "FunnyGuilds " + ChatColor.GRAY + "przeladowano! (" + ChatColor.AQUA + diff + "s" + ChatColor.GRAY + ")")
    }
