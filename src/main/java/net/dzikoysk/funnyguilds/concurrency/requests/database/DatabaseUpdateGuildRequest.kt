package net.dzikoysk.funnyguilds.concurrency.requests.databaseimport
...
class DatabaseUpdateGuildRequest(private val guild: Guild) : DefaultConcurrencyRequest() {
    override fun execute() {
        val dataModel: DataModel = FunnyGuilds.Companion.getInstance().getDataModel()
        try {
            if (dataModel is SQLDataModel) {
                DatabaseGuild.save(guild)
                if (FunnyGuilds.Companion.getInstance().getPluginConfiguration().regionsEnabled) {
                    DatabaseRegion.save(guild.region)
                }
                Stream.concat(guild.members.stream(), Stream.of(guild.owner)).forEach { obj: User? -> DatabaseUser.save() }
                return
            }
            if (dataModel is FlatDataModel) {
                val flatGuild = FlatGuild(guild)
                flatGuild.serialize(dataModel as FlatDataModel)
                if (FunnyGuilds.Companion.getInstance().getPluginConfiguration().regionsEnabled) {
                    val flatRegion = FlatRegion(guild.region)
                    flatRegion.serialize(dataModel as FlatDataModel)
                }
                Stream.concat(guild.members.stream(), Stream.of(guild.owner))
                    .map { user: User? -> FlatUser(user) }.forEach { flatUser: FlatUser -> flatUser.serialize(dataModel as FlatDataModel) }
            }
        } catch (th: Throwable) {
            FunnyGuilds.Companion.getPluginLogger().error("Could not update guild", th)
        }
    }
}