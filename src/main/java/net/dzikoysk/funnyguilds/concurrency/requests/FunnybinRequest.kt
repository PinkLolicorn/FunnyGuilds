package net.dzikoysk.funnyguilds.concurrency.requestsimport
...
class FunnybinRequest private constructor(private val sender: CommandSender, private val files: List<String>) : DefaultConcurrencyRequest() {
    override fun execute() {
        val sentPastes: MutableList<FunnybinResponse?> = ArrayList()
        for (i in files.indices) {
            val fileName = files[i]
            var file: File?
            var content: String? = null
            var type = PasteType.OTHER
            sender.sendMessage(ChatColor.GREEN.toString() + "Wysylam plik: " + ChatColor.AQUA + (i + 1) + ChatColor.GREEN + "/" + ChatColor.AQUA + files.size + ChatColor.GREEN + "...")
            if ("log" == fileName) {
                ...
                val config = ConfigHelper.loadConfig(FunnyGuilds.Companion.getInstance().getPluginConfigurationFile(), PluginConfiguration::class.java)
            ...
            if (content == null) {
                content = try {
                    ...
                    sender.sendMessage(ChatColor.RED.toString() + "Podany plik: " + fileName + " nie istnieje")
                    continue
                } catch (e: IOException) {
                    sender.sendMessage(ChatColor.RED.toString() + "Podany plik: " + fileName + " nie mogl być otworzony (szczegoly w konsoli)")
                    FunnyGuilds.Companion.getPluginLogger().error("Failed to open a file: $fileName", e)
                    continue
                }
            }
            try {
                ...
                sender.sendMessage(ChatColor.RED.toString() + "Podany plik: " + fileName + " nie mogl byc wyslany (szczegoly w konsoli)")
                FunnyGuilds.Companion.getPluginLogger().error("Failed to submit a paste: $fileName", e)
            }
        }
        if (sentPastes.size == 1) {
            sender.sendMessage(ChatColor.GREEN.toString() + "Plik wyslany. Link: " + ChatColor.AQUA + sentPastes[0].getShortUrl())
            return
        }
        sender.sendMessage(ChatColor.GREEN.toString() + "Tworze paczke z wyslanych plikow...")
        try {
            val response = FunnyTelemetry.createBundle(sentPastes.stream().map { obj: FunnybinResponse? -> obj.getUuid() }.collect(Collectors.toList()))
            sender.sendMessage(ChatColor.GREEN.toString() + "Paczka wyslana. Link: " + ChatColor.AQUA + response.shortUrl)
        } catch (e: IOException) {
            sender.sendMessage(ChatColor.RED.toString() + "Wystapil blad podczas tworzenia paczki. ")
            FunnyGuilds.Companion.getPluginLogger().error("Failed to submit a bundle. Files: $files", e)
        }
    }

    companion object {
        private fun ofData(sender: CommandSender, args: Array<String>): FunnybinRequest? {
            if (args.size == 1) {
                return FunnybinRequest(sender, Arrays.asList("config", "log"))
            }
            if (args.size >= 2) {
                when (args[1]) {
                    "config" -> return FunnybinRequest(sender, listOf("config"))
                    "log" -> return FunnybinRequest(sender, listOf("log"))
                    "custom" -> if (args.size >= 3) {
                        FunnybinRequest(sender, listOf(args[2]))
                    }
                    "bundle" -> if (args.size >= 3) {
                        return FunnybinRequest(sender, Arrays.asList(*args).subList(2, args.size))
                    }
                }
            }
            return null
        ...
        fun of(sender: CommandSender?, args: Array<String?>?): Optional<FunnybinRequest> {
            return Optional.ofNullable(FunnybinRequest.Companion.ofData(sender, args))
        }
    }
}