...
class ConcurrencyTaskBuilder {
    private val requests: List<ConcurrencyRequest>
    fun delegate(vararg delegatedRequests: ConcurrencyRequest?): ConcurrencyTaskBuilder {
        Collections.addAll(requests, *delegatedRequests)
        return this
    }

    fun build(): ConcurrencyTask {
        val requestArray = arrayOfNulls<ConcurrencyRequest>(requests.size)
        return ConcurrencyTask(*requests.toArray(requestArray))
    }
