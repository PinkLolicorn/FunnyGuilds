...
class ConcurrencyTask(vararg requests: ConcurrencyRequest) : Runnable {
    private val id: Int
    private val requests: Array<ConcurrencyRequest>
    override fun run() {
        for (request in requests) {
            val result = execute(request)
            if (!result) {
                FunnyGuilds.Companion.getPluginLogger().warning("Task #$id has been interrupted")
                return
            }
        }
    }

    private fun execute(request: ConcurrencyRequest): Boolean {
        try {
            request.execute()
        } catch (exception: Exception) {
            if (request.exceptionHandler != null) {
                request.exceptionHandler.handleException(exception)
            }
            if (!request.isMuted) {
                return false
            }
    ...
    companion object {
        ...
        fun builder(): ConcurrencyTaskBuilder {
            return ConcurrencyTaskBuilder()
        }
    }

    init {
        id = ConcurrencyTask.Companion.idAssigner.getAndIncrement()
