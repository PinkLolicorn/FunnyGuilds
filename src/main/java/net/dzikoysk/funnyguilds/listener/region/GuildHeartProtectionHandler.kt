...
class GuildHeartProtectionHandler : Listener {
    @EventHandler
    fun onPistonExtend(event: BlockPistonExtendEvent) {
        for (block in event.blocks) {
            if (GuildHeartProtectionHandler.Companion.isGuildHeart(block)) {
    ...
    fun onBlockBurn(event: BlockBurnEvent) {
        if (GuildHeartProtectionHandler.Companion.isGuildHeart(event.block)) {
            event.isCancelled = true
        }
    }
    ...
    companion object {
        fun isGuildHeart(block: Block): Boolean {
            val md: Pair<Material, Byte> = FunnyGuilds.Companion.getInstance().getPluginConfiguration().createMaterial
            if (md == null || block.type != md.left) {
                return false
            }
            val loc = block.location
            val region = RegionUtils.getAt(loc) ?: return false
            return block.location == region.heart
        }
    }
}