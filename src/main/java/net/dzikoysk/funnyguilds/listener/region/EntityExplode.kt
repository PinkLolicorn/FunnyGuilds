...
class EntityExplode : Listener {
    private val informationMessageCooldowns = Cooldown<Player>()
    @EventHandler(priority = EventPriority.LOWEST)
    fun preNormalExplosionHandler(event: EntityExplodeEvent) {
        val explodedBlocks = event.blockList()
        val explodeLocation = event.location
        val config: PluginConfiguration = FunnyGuilds.Companion.getInstance().getPluginConfiguration()
        val explosiveMaterials = config.explodeMaterials
        val blocksInSphere = SpaceUtils.sphereBlocks(
            explodeLocation,
        ...
        val explosionEntity = event.entity
        ...
        if (config.explodeShouldAffectOnlyGuild) {
            explodedBlocks.removeIf { block: Block? ->
                val region = RegionUtils.getAt(block!!.location)
                (region == null || region.guild == null) && block.type != Material.TNT
            }
            blocksInSphere!!.removeIf { block: Block? ->
                val region = RegionUtils.getAt(block!!.location)
                region == null || region.guild == null
            }
        }
        val region = RegionUtils.getAt(explodeLocation)
        if (region != null) {
            val guild = region.guild
            if (config.warTntProtection && !guild!!.canBeAttacked()) {
                event.isCancelled = true
                if (explosionEntity is TNTPrimed) {
                    val explosionSource = explosionEntity.source
                    if (explosionSource is Player) {
                        explosionSource.sendMessage(messages.regionExplosionHasProtection)
                    }
                }
                return
            }
            val guildHeartLocation = region.heart
            explodedBlocks.removeIf { block: Block? -> block!!.location == guildHeartLocation }
            blocksInSphere!!.removeIf { block: Block? -> block!!.location == guildHeartLocation }
            guild!!.build = System.currentTimeMillis() + config.regionExplode * 1000L
            for (user in guild.members) {
                val player = user.player
                if (player != null && !informationMessageCooldowns.cooldown(player, TimeUnit.SECONDS, config.infoPlayerCooldown.toLong())) {
                    player.sendMessage(FunnyGuilds.Companion.getInstance().getMessageConfiguration().regionExplode.replace("{TIME}", Integer.toString(config.regionExplode)))
                }
            }
        }
        if (config.warTntProtection) {
            val anyRemoved = explodedBlocks.removeIf { block: Block? ->
                val regionAtExplosion = RegionUtils.getAt(block!!.location)
                regionAtExplosion != null && regionAtExplosion.guild != null && !regionAtExplosion.guild!!.canBeAttacked()
            } || blocksInSphere!!.removeIf { block: Block? ->
                val regionAtExplosion = RegionUtils.getAt(block!!.location)
                regionAtExplosion != null && regionAtExplosion.guild != null && !regionAtExplosion.guild!!.canBeAttacked()
            }
        ...
        val additionalExplodedBlocks: MutableList<Block?> = ArrayList()
        for (block in blocksInSphere!!) {
            if (block!!.type == Material.TNT) {
                // We want to preserve TNT chain explosions, see GH-1414.
                continue
            }
            val material = block.type
            var explodeChance = explosiveMaterials!![material]
        ...
        if (!SimpleEventHandler.handle(GuildEntityExplodeEvent(EventCause.UNKNOWN, additionalExplodedBlocks))) {
            event.isCancelled = true
            return
        }
        additionalExplodedBlocks.stream()
            .filter { block: Block? -> !explodedBlocks.contains(block) }
            .forEach { e: Block? -> explodedBlocks.add(e) }
    }
}