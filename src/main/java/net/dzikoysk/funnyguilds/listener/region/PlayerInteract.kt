...
class PlayerInteract : Listener {
    private val infoExecutor = InfoCommand()
    ...
    fun onInteract(event: PlayerInteractEvent) {
        ...
        val region = RegionUtils.getAt(clicked.location) ?: return
        val heart = region.center.block.getRelative(BlockFace.DOWN)
        if (clicked == heart) {
            if (heart.type == Material.DRAGON_EGG) {
            ...
            if (!config.informationMessageCooldowns.cooldown(player, TimeUnit.SECONDS, config.infoPlayerCooldown.toLong())) {
                try {
                    infoExecutor.execute(config, FunnyGuilds.Companion.getInstance().getMessageConfiguration(), player, arrayOf(guild.tag))
        ...
        if (eventAction == Action.RIGHT_CLICK_BLOCK) {
            ...
            val blocked = config.blockedInteract!!.contains(clicked.type)
            if (guild.members.contains(user)) {
                event.isCancelled = blocked && config.regionExplodeBlockInteractions && !guild.canBuild()
            } else {
                event.isCancelled = blocked && !player.hasPermission("funnyguilds.admin.interact")
            }
        }
    }
}