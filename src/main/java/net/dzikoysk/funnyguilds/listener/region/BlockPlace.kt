...
class BlockPlace : Listener {
    private val config: PluginConfiguration = FunnyGuilds.Companion.getInstance().getPluginConfiguration()
    ...
    fun onPlace(event: BlockPlaceEvent) {
        ...
        player.sendMessage(FunnyGuilds.Companion.getInstance().getMessageConfiguration().regionOther)
        ...
        val returnItem = itemInHand.clone()
        returnItem.amount = 1

        // remove one item from the player's inventory
        if (itemInHand.amount - 1 == 0) {
        ...
        val sameColumn = playerLocation.blockX == blockLocation.blockX && playerLocation.blockZ == blockLocation.blockZ
        val distanceUp = playerLocation.y - blockLocation.blockY
        val upToTwoBlocks = distanceUp > 0 && distanceUp <= 2
        if (sameColumn && upToTwoBlocks) {
            player.velocity = BlockPlace.Companion.ANTI_GLITCH_VELOCITY
        }
        ...
        Bukkit.getScheduler().runTask(FunnyGuilds.Companion.getInstance(), Runnable {
            ...
            Bukkit.getScheduler().runTaskLater(FunnyGuilds.Companion.getInstance(), Runnable {
                event.blockReplacedState.update(true)
                if (!player.isOnline) {
                    return@runTaskLater
                }
    ...
    companion object {
        private val ANTI_GLITCH_VELOCITY: Vector = Vector(0, 0.4, 0)
    }
}