...
class PlayerCommand : Listener {
    ...
    fun onCommand(event: PlayerCommandPreprocessEvent) {
        ...
        val commandElements = event.message.split("\\s+".toRegex()).toTypedArray()
        if (commandElements.size == 0) {
            return
        }
        var command: String? = commandElements[0]
        for (blockedCommand in FunnyGuilds.Companion.getInstance().getPluginConfiguration().regionCommands) {
            if ("/$blockedCommand".equals(command, ignoreCase = true)) {
                command = null
                break
            }
        }
        ...
        val region = RegionUtils.getAt(player.location) ?: return
        ...
        player.sendMessage(FunnyGuilds.Companion.getInstance().getMessageConfiguration().regionCommand)
    }
}