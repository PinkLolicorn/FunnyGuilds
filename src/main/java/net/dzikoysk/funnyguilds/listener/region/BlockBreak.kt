...
class BlockBreak : Listener {
    ...
    fun onBreak(event: BlockBreakEvent) {
        if (ProtectionSystem.isProtected(event.player, event.block.location, FunnyGuilds.Companion.getInstance().getPluginConfiguration().regionExplodeBlockBreaking)) {
            event.isCancelled = true
            event.player.sendMessage(FunnyGuilds.Companion.getInstance().getMessageConfiguration().regionOther)
        }
    }
}