...
class TntProtection : Listener {
    ...
    fun onExplode(event: EntityExplodeEvent) {
        ...
        if (config.guildTNTProtectionEnabled) {
            val region = RegionUtils.getAt(event.location)
            if (region != null) {
                event.isCancelled = true
                return
            }
            val affectedBlocks = event.blockList().iterator()
            while (affectedBlocks.hasNext()) {
                val block = affectedBlocks.next()
                val maybeRegion = RegionUtils.getAt(block.location)
                if (maybeRegion != null) {
