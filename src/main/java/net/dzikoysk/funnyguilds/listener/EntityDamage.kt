...
import net.dzikoysk.funnyguilds.util.commons.bukkit.*
...
class EntityDamage : Listener {
    ...
    fun onDamage(event: EntityDamageByEntityEvent) {
        EntityUtils.getAttacker(event.damager).peek { attacker: Player? ->
            val config: PluginConfiguration = FunnyGuilds.Companion.getInstance().getPluginConfiguration()
            val attackerUser: User = User.Companion.get(attacker)
            val victim = event.entity
            if (config.animalsProtection && (victim is Animals || victim is Villager)) {
                RegionUtils.getAtOpt(victim.location)
                    .map { obj: Region? -> obj!!.guild }
                    .filterNot { guild: Guild? -> guild == attackerUser.guild }
                    .peek { guild: Guild? -> event.isCancelled = true }
                return@peek
            }
            if (victim !is Player) {
                return@peek
            }
            val victimUser: User = User.Companion.get(event.entity as Player)
            if (victimUser.hasGuild() && attackerUser.hasGuild()) {
                if (victimUser.uuid == attackerUser.uuid) {
                    return@peek
                }
                if (victimUser.guild == attackerUser.guild) {
                    if (!victimUser.guild!!.pvP) {
                        event.isCancelled = true
                        return@peek
                    }
                }
                if (victimUser.guild!!.allies!!.contains(attackerUser.guild)) {
                    if (!config.damageAlly) {
                        event.isCancelled = true
                        return@peek
                    }
                    if (!(attackerUser.guild!!.getPvP(victimUser.guild) && victimUser.guild!!.getPvP(attackerUser.guild))) {
                        event.isCancelled = true
                        return@peek
                    }
                }
            ...
            victimUser.cache.addDamage(attackerUser, event.damage)
        }
    }
}