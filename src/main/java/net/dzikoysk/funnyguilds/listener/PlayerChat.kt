...
class PlayerChat : Listener {
        ...
        if (user.hasGuild()) {
            ...
            if (sendGuildMessage(event, message, config, player, guild)) {
                if (config.logGuildChat) {
                    FunnyGuilds.Companion.getPluginLogger().info("[Guild Chat] $message")
                }
                return
            }
        }
        val points = user.rank.points
        var format: String? = event.format
        format = StringUtils.replace(format, "{RANK}", StringUtils.replace(config.chatRank, "{RANK}", user.rank.position.toString()))
        format = StringUtils.replace(format, "{POINTS}", config.chatPoints)
        format = StringUtils.replace(format, "{POINTS-FORMAT}", IntegerRange.Companion.inRangeToString<String?>(points, config.pointsFormat))
        format = StringUtils.replace(format, "{POINTS}", points.toString())
        if (user.hasGuild()) {
            format = StringUtils.replace(format, "{TAG}", StringUtils.replace(config.chatGuild, "{TAG}", user.guild.tag))
    ...
    private fun sendGuildMessage(event: AsyncPlayerChatEvent, message: String, config: PluginConfiguration, player: Player, guild: Guild?): Boolean {
        if (sendMessageToAllGuilds(event, message, config, player, guild)) {
            return true
        }
        return if (sendMessageToGuildAllies(event, message, config, player, guild)) {
            true
        } else sendMessageToGuildMembers(event, message, config, player, guild)
    }

    private fun spy(player: Player, message: String) {
        val spyMessage = ChatColor.GOLD.toString() + "[Spy] " + ChatColor.GRAY + player.name + ": " + ChatColor.WHITE + message
        for (looped in Bukkit.getOnlinePlayers()) {
            if (User.Companion.get(looped).getCache().isSpy()) {
                looped.sendMessage(spyMessage)
            }
        }
    }

    private fun sendMessageToGuildMembers(event: AsyncPlayerChatEvent, message: String, config: PluginConfiguration, player: Player, guild: Guild?): Boolean {
        val guildPrefix = config.chatPriv
        val prefixLength = guildPrefix!!.length
        if (message.length > prefixLength && message.substring(0, prefixLength) == guildPrefix) {
            var resultMessage = config.chatPrivDesign
            resultMessage = StringUtils.replace(resultMessage, "{PLAYER}", player.name)
            resultMessage = StringUtils.replace(resultMessage, "{TAG}", guild.getTag())
            resultMessage = StringUtils.replace(
                resultMessage, "{POS}",
                StringUtils.replace(config.chatPosition, "{POS}", getPositionString(User.Companion.get(player), config))
            )
            val messageWithoutPrefix = event.message.substring(prefixLength).trim { it <= ' ' }
            resultMessage = StringUtils.replace(resultMessage, "{MESSAGE}", messageWithoutPrefix)
            spy(player, messageWithoutPrefix)
            sendMessageToGuild(guild, player, resultMessage)
    ...
    private fun sendMessageToGuildAllies(event: AsyncPlayerChatEvent, message: String, config: PluginConfiguration, player: Player, guild: Guild?): Boolean {
        val allyPrefix = config.chatAlly
        val prefixLength = allyPrefix!!.length
        if (message.length > prefixLength && message.substring(0, prefixLength) == allyPrefix) {
            var resultMessage = config.chatAllyDesign
            resultMessage = StringUtils.replace(resultMessage, "{PLAYER}", player.name)
            resultMessage = StringUtils.replace(resultMessage, "{TAG}", guild.getTag())
            resultMessage = StringUtils.replace(
                resultMessage, "{POS}",
            ...
            val subMessage = event.message.substring(prefixLength).trim { it <= ' ' }
            resultMessage = StringUtils.replace(resultMessage, "{MESSAGE}", subMessage)
            spy(player, subMessage)
            sendMessageToGuild(guild, player, resultMessage)
            for (ally in guild!!.allies!!) {
                sendMessageToGuild(ally, player, resultMessage)
            }
            event.isCancelled = true
            return true
        }
    ...
    private fun sendMessageToAllGuilds(event: AsyncPlayerChatEvent, message: String, config: PluginConfiguration, player: Player, guild: Guild?): Boolean {
        val allGuildsPrefix = config.chatGlobal
        val prefixLength = allGuildsPrefix!!.length
        if (message.length > prefixLength && message.substring(0, prefixLength) == allGuildsPrefix) {
            var resultMessage = config.chatGlobalDesign
            resultMessage = StringUtils.replace(resultMessage, "{PLAYER}", player.name)
            resultMessage = StringUtils.replace(resultMessage, "{TAG}", guild.getTag())
            resultMessage = StringUtils.replace(
                resultMessage, "{POS}",
            ...
            resultMessage = StringUtils.replace(resultMessage, "{MESSAGE}", subMessage)
            spy(player, subMessage)
            for (globalGuild in GuildUtils.getGuilds()) {
                sendMessageToGuild(globalGuild, player, resultMessage)
            }
            event.isCancelled = true
            return true
        }
    ...
    private fun sendMessageToGuild(guild: Guild?, player: Player?, message: String?) {
        if (guild == null || player == null || !player.isOnline) {
            return
        }
        for (onlineMember in guild.onlineMembers) {
            val member = onlineMember.player ?: continue
            if (member != player || !onlineMember.cache.isSpy) {
                member.sendMessage(message!!)
            }
        }
    }

    private fun getPositionString(user: User?, config: PluginConfiguration): String? {
        if (user!!.isOwner) {
            return config.chatPositionLeader
        }
        return if (user.isDeputy) {
            config.chatPositionDeputy
        } else config.chatPositionMember
    }
}