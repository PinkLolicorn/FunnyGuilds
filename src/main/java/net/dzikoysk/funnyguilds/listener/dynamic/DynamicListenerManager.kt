package net.dzikoysk.funnyguilds.listener.dynamicimport
...
class DynamicListenerManager(private val funnyGuilds: FunnyGuilds) {
    private val registrations: MutableSet<DynamicListenerRegistration> = HashSet()
    fun registerDynamic(predicate: Supplier<Boolean>, vararg listeners: Listener?): Boolean {
        val registration = DynamicListenerRegistration(funnyGuilds, Arrays.asList(*listeners), predicate)
        registration.reload()
        return registrations.add(registration)
    }

    fun unregister(listener: Listener?): Boolean {
        val registrationIterator = registrations.iterator()
        var result = false
        while (registrationIterator.hasNext()) {
            val next = registrationIterator.next()
    ...
    fun reloadAll() {
        registrations.forEach(Consumer { obj: DynamicListenerRegistration -> obj.reload() })
    }

    fun unregisterAll() {
        registrations.forEach(Consumer { obj: DynamicListenerRegistration -> obj.forceUnregister() })
        registrations.clear()
    }
}