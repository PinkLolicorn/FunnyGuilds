...
internal class DynamicListenerRegistration(private val funnyGuilds: FunnyGuilds, listeners: Collection<Listener>, predicate: Supplier<Boolean>) {
    val listeners: Collection<Listener>
    private val predicate: Supplier<Boolean>
    private var currentState = false
    fun reload() {
        val requiredState = predicate.get()
    ...
    fun forceRegister() {
        listeners.forEach(Consumer { listener: Listener? -> funnyGuilds.server.pluginManager.registerEvents(listener!!, funnyGuilds) })
        currentState = true
    }

    fun forceUnregister() {
        listeners.forEach(Consumer { listener: Listener? -> HandlerList.unregisterAll(listener!!) })
        currentState = false
    }
