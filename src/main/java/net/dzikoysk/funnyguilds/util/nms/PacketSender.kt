package net.dzikoysk.funnyguilds.util.nmsimport
...
object PacketSender {
    private val getHandle: Method? = null
    private val playerConnection: Field? = null
    private val sendPacket: Method? = null
    fun sendPacket(players: Collection<Player?>, vararg packets: Any?) {
        val packetList = Arrays.asList(*packets)
        players.forEach { p: Player? -> PacketSender.sendPacket(p, packetList) }
    }

    fun sendPacket(players: Array<Player?>, vararg packets: Any?) {
        val packetList = Arrays.asList(*packets)
    ...
    fun sendPacket(target: Player?, vararg packets: Any?) {
        PacketSender.sendPacket(target, Arrays.asList(*packets))
    }

    fun sendPacket(packets: List<Any?>?) {
        Bukkit.getOnlinePlayers().forEach { p: Player? -> PacketSender.sendPacket(p, packets) }
    }

    fun sendPacket(target: Player?, packets: List<Any?>) {
        if (target == null) {
            return
        }
        try {
            val handle = PacketSender.getHandle.invoke(target)
            val connection = PacketSender.playerConnection[handle]
            for (packet in packets) {
                PacketSender.sendPacket.invoke(connection, packet)
            }
        } catch (exception: IllegalAccessException) {
            FunnyGuilds.Companion.getPluginLogger().error("Failed to send packets", exception)
    ...
    init {
        try {
            ...
            FunnyGuilds.Companion.getPluginLogger().error("Could not initialize PacketSender class", ex)
        }
    }
}