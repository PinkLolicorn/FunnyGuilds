package net.dzikoysk.funnyguilds.util.nmsimportimport
...
object BlockDataChanger {
    private val craftBlockClass: Class<*>? = null
    private val setDataMethod: Method? = null
    fun applyChanges(targetBlock: Block, newData: Byte) {
        if (!Reflections.USE_PRE_13_METHODS) {
            return
        }
        try {
            BlockDataChanger.setDataMethod.invoke(targetBlock, newData)
        } catch (ex: IllegalAccessException) {
            FunnyGuilds.Companion.getPluginLogger().error("Failed to change block data for a block at: " + LocationUtils.toString(targetBlock.location), ex)
    ...
    init {
        BlockDataChanger.craftBlockClass = Reflections.getCraftBukkitClass("block.CraftBlock")
        BlockDataChanger.setDataMethod = Reflections.getMethod(BlockDataChanger.craftBlockClass, "setData", Byte::class.javaPrimitiveType!!)
    }
}