package net.dzikoysk.funnyguilds.util.commonsimport
...
object MapUtil {
    fun <K : Comparable<K>?, V> sortByKey(map: Map<K, V>, descending: Boolean): Map<K, V> {
        var stream: Stream<Entry<K, V>?> = map.entries.stream()
        stream = if (descending) {
            stream.sorted(java.util.Map.Entry.comparingByKey(Collections.reverseOrder()))
        } else {
            stream.sorted(java.util.Map.Entry.comparingByKey())
        }
        return stream.collect(
            Collectors.toMap(
                Function<Entry<K, V>, K> { obj: Entry<K, V> -> obj.key },
                Function<Entry<K, V>, V> { obj: Entry<K, V> -> obj.value },
                BinaryOperator { firstEntry: V, secondEntry: V -> secondEntry },
                Supplier { LinkedHashMap() })
        )
    }

    fun <K : Comparable<K>?, V> sortByKey(map: Map<K, V>?): Map<K, V> {
        return MapUtil.sortByKey(map, true)
    }

    fun <K, V : Comparable<V>?> sortByValue(map: Map<K, V>, descending: Boolean): Map<K, V> {
        var stream: Stream<Entry<K, V>?> = map.entries.stream()
        stream = if (descending) {
            stream.sorted(java.util.Map.Entry.comparingByValue(Collections.reverseOrder()))
        } else {
            stream.sorted(java.util.Map.Entry.comparingByValue())
        }
        return stream.collect(
            Collectors.toMap(
                Function<Entry<K, V>, K> { obj: Entry<K, V> -> obj.key },
                Function<Entry<K, V>, V> { obj: Entry<K, V> -> obj.value },
                BinaryOperator { firstEntry: V, secondEntry: V -> firstEntry },
                Supplier { LinkedHashMap() })
        )
    }
    ...
    fun <K : Comparable<K>?, V> findTheMaximumEntryByKey(map: Map<K, V>): Entry<K, V> {
        return map.entries.stream().max(Comparator.comparing(Function<Entry<K, V>, K> { obj: Entry<K, V> -> obj.key })).orElse(null)
    }

    fun <K, V : Comparable<V>?> findTheMaximumEntryByValue(map: Map<K, V>): Entry<K, V> {
        return map.entries.stream().max(Comparator.comparing(Function<Entry<K, V>, V> { obj: Entry<K, V> -> obj.value })).orElse(null)
    }

    fun <K : Comparable<K>?, V> findTheMinimumEntryByKey(map: Map<K, V>): Entry<K, V> {
        return map.entries.stream().min(Comparator.comparing(Function<Entry<K, V>, K> { obj: Entry<K, V> -> obj.key })).orElse(null)
    }

    fun <K, V : Comparable<V>?> findTheMinimumEntryByValue(map: Map<K, V>): Entry<K, V> {
        return map.entries.stream().min(Comparator.comparing(Function<Entry<K, V>, V> { obj: Entry<K, V> -> obj.value })).orElse(null)
    }
}