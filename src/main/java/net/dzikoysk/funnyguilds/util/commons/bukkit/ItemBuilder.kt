package net.dzikoysk.funnyguilds.util.commons.bukkitimport
...
class ItemBuilder {
    ...
    constructor(material: Material?, stack: Int) {
        item = ItemStack(material!!, stack)
        meta = item.itemMeta
    }

    constructor(material: Material?, stack: Int, data: Int) {
        item = ItemStack(material!!, stack, data.toShort())
        meta = item.itemMeta
    }
    ...
    fun setName(name: String?, color: Boolean): ItemBuilder {
        meta!!.setDisplayName(if (color) ChatUtils.colored(name) else name)
        refreshMeta()
        return this
    }
    ...
    fun setFlag(flag: ItemFlag?): ItemBuilder {
