...
object LoggingUtils {
    fun flushRootLogger() {
        LoggingUtils.flushLogger(LogManager.getRootLogger() as Logger)
    }

    fun flushLogger(rootLogger: Logger) {
        rootLogger.appenders.values
            .stream()
            .filter { appender: Appender? -> appender is AbstractOutputStreamAppender<*> }
            .map { appender: Appender? -> appender as AbstractOutputStreamAppender<*>? }
            .filter(Predicate { appender: AbstractOutputStreamAppender<*> -> !appender.immediateFlush })
            .forEach(Consumer { appender: AbstractOutputStreamAppender<*> -> appender.manager.flush() })
    }
}