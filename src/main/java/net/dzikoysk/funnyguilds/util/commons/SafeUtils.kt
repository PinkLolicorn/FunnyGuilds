...
object SafeUtils {
    private fun reportUnsafe(th: Throwable) {
        FunnyGuilds.Companion.getPluginLogger().error("Something went wrong while handling unsafe", th)
    }

    fun <T> safeInit(initializer: SafeInitializer<T>): T? {
        return try {
            initializer.initialize()
        } catch (e: Exception) {
            SafeUtils.reportUnsafe(e)
            null
        }
    }

    fun interface SafeInitializer<T> {
