...
class FunnyGuildsLogger internal constructor(private val funnyGuilds: FunnyGuilds) {
    private val rootLogger: Logger
    fun update(content: String) {
        rootLogger.info("[Updater] > $content")
    }

    fun parser(content: String) {
        rootLogger.warning("[Parser] > $content")
    }
    ...
    fun debug(content: String) {
        if (funnyGuilds.pluginConfiguration.debugMode) {
            rootLogger.info("[Debug] > $content")
        }
    }
    ...
    fun error(content: String?, cause: Throwable) {
        var loadedPlugins = Arrays.stream(Bukkit.getPluginManager().plugins)
            .filter { plugin: Plugin -> !plugin.name.contains("FunnyGuilds") }
            .map { plugin: Plugin -> plugin.name + " " + plugin.description.version }
        ...
        error("  FunnyGuilds: " + funnyGuilds.version.fullVersion)
