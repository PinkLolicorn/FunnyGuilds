...
object IndividualPrefixManager {
    fun updatePlayers() {
    ...
    fun updatePlayer(player: Player) {
        ...
        val cachedScoreboard = cache.scoreboard
        ...
        if (cachedScoreboard == null) {
            ...
            Bukkit.getScheduler().runTask(FunnyGuilds.Companion.getInstance(), Runnable {
                ...
                cache.dummy.updateScore(user)
            })
            return
        }
        try {
            player.scoreboard = cachedScoreboard
        } catch (e: IllegalStateException) {
            FunnyGuilds.Companion.getPluginLogger().warning("[IndividualPrefix] java.lang.IllegalStateException: Cannot set scoreboard for invalid CraftPlayer (" + player.javaClass + ")")
        }
    }

    fun addGuild(to: Guild?) {
        for (player in Bukkit.getOnlinePlayers()) {
            val prefix: IndividualPrefix = User.Companion.get(player).getCache().getIndividualPrefix() ?: continue
            prefix.addGuild(to)
        }
    ...
    fun addPlayer(player: String?) {
        for (p in Bukkit.getOnlinePlayers()) {
            val prefix: IndividualPrefix = User.Companion.get(p).getCache().getIndividualPrefix() ?: continue
    ...
    fun removePlayer(player: String?) {
        for (ps in Bukkit.getOnlinePlayers()) {
            val prefix: IndividualPrefix = User.Companion.get(ps).getCache().getIndividualPrefix() ?: continue
