package net.dzikoysk.funnyguilds.element.notificationimport
...
object NotificationUtil {
    private val PACKET_PLAY_OUT_TITLE_CLASS: Class<*>? = null
    private val PACKET_PLAY_OUT_CHAT_CLASS: Class<*>? = null
    private val TITLE_ACTION_CLASS: Class<*>? = null
    private val CHAT_MESSAGE_TYPE_CLASS: Class<*>? = null
    private val CREATE_BASE_COMPONENT_NMS: Method? = null
    private val CREATE_BASE_COMPONENT_CRAFTBUKKIT: Method? = null
    private val GET_TITLE_ACTION_ENUM: Method? = null
    private val TITLE_ENUM: Enum<*>? = null
    private val SUBTITLE_ENUM: Enum<*>? = null
    private val TIMES_ENUM: Enum<*>? = null
    private const val BASE_COMPONENT_JSON_PATTERN = "{\"text\": \"{TEXT}\"}"
    private val SENDER_ALWAYS_DISPLAY = UUID(0L, 0L)
    fun createTitleNotification(text: String?, subText: String?, fadeIn: Int, stay: Int, fadeOut: Int): List<Any?> {
        val packets: MutableList<Any?> = Lists.newArrayList()
        val titlePacket: Any = PacketCreator.Companion.of(NotificationUtil.PACKET_PLAY_OUT_TITLE_CLASS)
            .create()
            .withField("a", NotificationUtil.TITLE_ENUM)
            .withField("b", NotificationUtil.createBaseComponent(text, false))
            .getPacket()
        val subtitlePacket: Any = PacketCreator.Companion.of(NotificationUtil.PACKET_PLAY_OUT_TITLE_CLASS)
            .create()
            .withField("a", NotificationUtil.SUBTITLE_ENUM)
            .withField("b", NotificationUtil.createBaseComponent(subText, false))
            .getPacket()
        val timesPacket: Any = PacketCreator.Companion.of(NotificationUtil.PACKET_PLAY_OUT_TITLE_CLASS)
            .create()
            .withField("a", NotificationUtil.TIMES_ENUM)
            .withField("c", fadeIn)
            .withField("d", stay)
            .withField("e", fadeOut)
            .getPacket()
    ...
    fun createBaseComponent(text: String?, keepNewLines: Boolean): Any? {
        val text0 = text ?: ""
        return try {
            if (keepNewLines) Array.get(NotificationUtil.CREATE_BASE_COMPONENT_CRAFTBUKKIT.invoke(null, text0, true), 0) else NotificationUtil.CREATE_BASE_COMPONENT_NMS.invoke(
                null,
                StringUtils.replace(NotificationUtil.BASE_COMPONENT_JSON_PATTERN, "{TEXT}", text0)
            )
        } catch (ex: IllegalAccessException) {
            FunnyGuilds.Companion.getPluginLogger().error("Could not create base component", ex)
            null
    ...
    fun createActionbarNotification(text: String?): Any? {
        val actionbarPacket: Any
        actionbarPacket = if (NotificationUtil.CHAT_MESSAGE_TYPE_CLASS != null) {
            val packetCreator: PacketCreator = PacketCreator.Companion.of(NotificationUtil.PACKET_PLAY_OUT_CHAT_CLASS)
                .create()
                .withField("a", NotificationUtil.createBaseComponent(text, false))
                .withField("b", NotificationUtil.CHAT_MESSAGE_TYPE_CLASS.enumConstants[2])
            when (Reflections.SERVER_VERSION) {
                "v1_16_R1", "v1_16_R2", "v1_16_R3" ->                     // We always want to display our action bar notification (and it only applies to 1.16+)
                    packetCreator.withField("c", NotificationUtil.SENDER_ALWAYS_DISPLAY)
                else -> {
                }
            }
            packetCreator.packet
        } else {
            PacketCreator.Companion.of(NotificationUtil.PACKET_PLAY_OUT_CHAT_CLASS)
                .create()
                .withField("a", NotificationUtil.createBaseComponent(text, false))
                .withField("b", 2.toByte())
                .getPacket()
        }
    ...
    init {
        NotificationUtil.PACKET_PLAY_OUT_TITLE_CLASS = Reflections.getNMSClass("PacketPlayOutTitle")
        NotificationUtil.PACKET_PLAY_OUT_CHAT_CLASS = Reflections.getNMSClass("PacketPlayOutChat")
        NotificationUtil.TITLE_ACTION_CLASS =
            if ("v1_8_R1" == Reflections.SERVER_VERSION) Reflections.getNMSClass("EnumTitleAction") else Reflections.getNMSClass("PacketPlayOutTitle\$EnumTitleAction")
        if (Reflections.USE_PRE_12_METHODS) {
            NotificationUtil.CHAT_MESSAGE_TYPE_CLASS = null
        } else {
            NotificationUtil.CHAT_MESSAGE_TYPE_CLASS = Reflections.getNMSClass("ChatMessageType")
        }
        NotificationUtil.CREATE_BASE_COMPONENT_NMS = if ("v1_8_R1" == Reflections.SERVER_VERSION) Reflections.getMethod(
            Reflections.getNMSClass("ChatSerializer"),
            "a",
            String::class.java
        ) else Reflections.getMethod(Reflections.getNMSClass("IChatBaseComponent\$ChatSerializer"), "a", String::class.java)
        NotificationUtil.CREATE_BASE_COMPONENT_CRAFTBUKKIT =
            Reflections.getMethod(Reflections.getCraftBukkitClass("util.CraftChatMessage"), "fromString", String::class.java, Boolean::class.javaPrimitiveType!!)
        NotificationUtil.GET_TITLE_ACTION_ENUM = Reflections.getMethod(NotificationUtil.TITLE_ACTION_CLASS, "valueOf", String::class.java)
        try {
            NotificationUtil.TITLE_ENUM = NotificationUtil.GET_TITLE_ACTION_ENUM.invoke(null, "TITLE") as Enum<*>
            NotificationUtil.SUBTITLE_ENUM = NotificationUtil.GET_TITLE_ACTION_ENUM.invoke(null, "SUBTITLE") as Enum<*>
            NotificationUtil.TIMES_ENUM = NotificationUtil.GET_TITLE_ACTION_ENUM.invoke(null, "TIMES") as Enum<*>
        } catch (ex: IllegalAccessException) {
            throw RuntimeException("Could not retrieve title packet enums", ex)
        } catch (ex: InvocationTargetException) {
            throw RuntimeException("Could not retrieve title packet enums", ex)
        }
    }
