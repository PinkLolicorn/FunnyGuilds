package net.dzikoysk.funnyguilds.element.notification.bossbar.providerimport
...
class BossBarOptions private constructor(val color: BarColor, val style: BarStyle, val flags: Set<BarFlag>) {

    class Builder {
        private var barColor = BarColor.RED
        private var barStyle = BarStyle.SOLID
        private val barFlags: MutableSet<BarFlag> = Sets.newHashSet()
        fun color(barColor: String?): BossBarOptions.Builder {
            for (loopColor in BarColor.values()) {
                if (loopColor.name.equals(barColor, ignoreCase = true)) {
        ...
        fun style(barStyle: String?): BossBarOptions.Builder {
            for (loopStyle in BarStyle.values()) {
                if (loopStyle.name.equals(barStyle, ignoreCase = true)) {
        ...
        fun flags(barFlags: Collection<String?>): BossBarOptions.Builder {
            for (flag in barFlags) {
                for (barFlag in BarFlag.values()) {
                    if (barFlag.name.equals(flag, ignoreCase = true)) {
        ...
        fun build(): BossBarOptions {
            return BossBarOptions(barColor, barStyle, barFlags)
        }
    }

    companion object {
        fun builder(): BossBarOptions.Builder {
            return BossBarOptions.Builder()
        }
    }
}