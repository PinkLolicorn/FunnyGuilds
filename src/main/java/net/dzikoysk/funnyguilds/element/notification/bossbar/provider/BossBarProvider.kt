...
interface BossBarProvider {
    fun sendNotification(text: String, options: BossBarOptions?, timeout: Int)
    fun removeNotification()

    companion object {
        fun getBossBar(user: User?): BossBarProvider? {
            return when (Reflections.SERVER_VERSION) {
                "v1_8_R1", "v1_8_R3" -> BossBarProviderImpl(user)
                else -> DefaultBossBarProvider(user)
            }
        }
    }
}