...
class DefaultBossBarProvider(private val user: User) : BossBarProvider {
    val bossBar: BossBar

    @Volatile
    private var hideBossBarTask: BukkitTask? = null
    override fun sendNotification(text: String, options: BossBarOptions?, timeout: Int) {
        val player = user.player ?: return
        bossBar.setTitle(text)
        bossBar.color = options!!.getColor()
        bossBar.style = options.getStyle()
        options.getFlags().forEach(Consumer { barFlag: BarFlag? -> bossBar.addFlag(barFlag!!) })
        bossBar.isVisible = true
        if (!bossBar.players.contains(player)) {
            bossBar.addPlayer(player)
        }
        if (hideBossBarTask != null) {
            hideBossBarTask!!.cancel()
        }
        hideBossBarTask = Bukkit.getScheduler().runTaskLaterAsynchronously(FunnyGuilds.Companion.getInstance(), Runnable {
