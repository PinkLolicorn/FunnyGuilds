package net.dzikoysk.funnyguilds.element.tablistimport
...
abstract class AbstractTablist(tablistPattern: Map<Int, String>, header: String, footer: String, ping: Int, user: User) {
    protected val variables = TablistVariablesParser()
    protected val tablistPattern: Map<Int, String>
    protected val header: String
    protected val footer: String
    protected val user: User
    protected var cells = 0
    protected val ping: Int
    protected var firstPacket = true
    abstract fun send()
    protected fun sendPackets(packets: List<Any?>?) {
        val target = user.player ?: return
        PacketSender.sendPacket(target, packets)
    }

    protected fun createBaseComponent(text: String?, keepNewLines: Boolean): Any {
        return NotificationUtil.createBaseComponent(text, keepNewLines)
    }

    protected fun putVarsPrepareCells(cells: Int, tablistPattern: Map<Int?, String?>, header: String?, footer: String?): Array<String> {
        val allCells = arrayOfNulls<String>(AbstractTablist.Companion.DEFAULT_CELLS_AMOUNT + 2) // Additional two for header/footer
        for (i in 0 until cells) {
            allCells[i] = putRank(tablistPattern.getOrDefault(i + 1, ""))
        }
        allCells[AbstractTablist.Companion.DEFAULT_CELLS_AMOUNT] = header
        allCells[AbstractTablist.Companion.DEFAULT_CELLS_AMOUNT + 1] = footer
        val mergedCells = StringUtils.join(allCells, '\u0000')
        return StringUtils.splitPreserveAllTokens(putVars(mergedCells), '\u0000')
    }

    protected fun putRank(cell: String?): String? {
        val temp = RankUtils.parseRank(user, cell)
        return temp ?: cell
    }

    protected fun putVars(cell: String?): String? {
        var formatted = cell
        val player = user.player ?: return formatted
        val result = variables.createResultFor(user)
    ...
    @Deprecated("")
    protected fun putHeaderFooterVars(text: String?): String? {
        var formatted = text
    ...
    protected fun shouldUseHeaderAndFooter(): Boolean {
        return !header.isEmpty() || !footer.isEmpty()
    }

    companion object {
        const val DEFAULT_CELLS_AMOUNT = 80
        private val TABLIST_CACHE: Map<UUID, AbstractTablist> = ConcurrentHashMap()
        fun wipeCache() {
            AbstractTablist.Companion.TABLIST_CACHE.clear()
            DefaultTablistVariables.clearFunnyVariables()
        }

        fun createTablist(pattern: Map<Int?, String?>?, header: String?, footer: String?, ping: Int, player: Player?): AbstractTablist {
            return AbstractTablist.Companion.createTablist(pattern, header, footer, ping, User.Companion.get(player))
        }

        fun createTablist(pattern: Map<Int?, String?>?, header: String?, footer: String?, ping: Int, user: User): AbstractTablist {
            var tablist: AbstractTablist = AbstractTablist.Companion.TABLIST_CACHE.get(user.uuid)
            if (tablist != null) {
                return tablist
            }
            tablist = when (Reflections.SERVER_VERSION) {
                "v1_8_R1" -> TablistImpl(pattern, header, footer, ping, user)
                "v1_8_R2", "v1_8_R3", "v1_9_R1", "v1_9_R2" -> net.dzikoysk.funnyguilds.element.tablist.impl.v1_8_R3.TablistImpl(pattern, header, footer, ping, user)
                "v1_10_R1", "v1_11_R1", "v1_12_R1", "v1_13_R1" -> net.dzikoysk.funnyguilds.element.tablist.impl.v1_10_R1.TablistImpl(pattern, header, footer, ping, user)
                "v1_13_R2", "v1_14_R1", "v1_15_R1", "v1_16_R1", "v1_16_R2", "v1_16_R3" -> net.dzikoysk.funnyguilds.element.tablist.impl.v1_13_R2.TablistImpl(pattern, header, footer, ping, user)
                else -> throw RuntimeException(String.format("Could not find tablist for given version: %s", Reflections.SERVER_VERSION))
            }
            AbstractTablist.Companion.TABLIST_CACHE.put(user.uuid, tablist)
            return tablist
        }

        fun getTablist(player: Player): AbstractTablist {
            val tablist: AbstractTablist = AbstractTablist.Companion.TABLIST_CACHE.get(player.uniqueId)
            if (tablist != null) {
                return tablist
            }
            throw IllegalStateException("Given player's tablist does not exist!")
        }

        fun removeTablist(player: Player) {
            AbstractTablist.Companion.TABLIST_CACHE.remove(player.uniqueId)
        }

        fun hasTablist(player: Player): Boolean {
            return AbstractTablist.Companion.TABLIST_CACHE.containsKey(player.uniqueId)
        }
    }
    ...
    init {
        ...
        if (!config.playerListFillCells) {
            val entry: Entry<Int, String?>? = MapUtil.findTheMaximumEntryByKey(config.playerList)
            if (entry != null) {
                cells = entry.key
            } else {
                cells = AbstractTablist.Companion.DEFAULT_CELLS_AMOUNT
            }
        } else {
            cells = AbstractTablist.Companion.DEFAULT_CELLS_AMOUNT
        }
        DefaultTablistVariables.install(variables)
    }
}