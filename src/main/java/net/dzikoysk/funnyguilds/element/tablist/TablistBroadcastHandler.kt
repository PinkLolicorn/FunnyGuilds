...
class TablistBroadcastHandler : Runnable {
    override fun run() {
        ...
        for (player in Bukkit.getOnlinePlayers()) {
            if (!AbstractTablist.Companion.hasTablist(player)) {
                val user: User = User.Companion.get(player) ?: continue
                AbstractTablist.Companion.createTablist(config.playerList, config.playerListHeader, config.playerListFooter, config.playerListPing, user)
            }
            val tablist: AbstractTablist = AbstractTablist.Companion.getTablist(player)
            tablist.send()
        }
    }
}