...
class TablistVariablesParser {
    private val tablistVariables: MutableCollection<TablistVariable> = ArrayList()
    fun add(variable: TablistVariable) {
        tablistVariables.add(variable)
    }

    fun createResultFor(user: User?): VariableParsingResult {
        val values: MutableMap<String, String> = HashMap()
        val currentTime = LocalDateTime.now()
        for (tablistVariable in tablistVariables) {
            if (tablistVariable is TimeFormattedVariable) {
                tablistVariable.provideCurrentTime(currentTime)
            }
            val value = tablistVariable[user]
            for (name in tablistVariable.names()) {
                if (values.containsKey(name)) {
                    FunnyGuilds.Companion.getPluginLogger().warning("Conflicting variable name: $name")
                }
                values[name] = value
            }
        }
        return VariableParsingResult(user, values)
    }
}