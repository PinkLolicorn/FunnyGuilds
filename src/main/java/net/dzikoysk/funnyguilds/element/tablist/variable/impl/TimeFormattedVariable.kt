...
class TimeFormattedVariable(private val names: Array<String>, private val function: BiFunction<User?, LocalDateTime?, Any?>) : TablistVariable {
    private var currentTime: LocalDateTime? = null

    constructor(name: String, function: BiFunction<User?, LocalDateTime?, Any?>) : this(arrayOf<String>(name), function) {}

    override fun names(): Array<String> {
        return names
    }

    fun provideCurrentTime(currentTime: LocalDateTime?) {
        this.currentTime = currentTime
    }

    override fun get(user: User): String? {
        return ChatUtils.appendDigit(Objects.toString(function.apply(user, if (currentTime != null) currentTime else LocalDateTime.now())))
    }
}