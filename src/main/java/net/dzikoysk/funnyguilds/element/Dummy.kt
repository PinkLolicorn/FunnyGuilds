...
class Dummy(private val user: User) {
    fun updateScore(user: User?) {
        if (!FunnyGuilds.Companion.getInstance().getPluginConfiguration().dummyEnable) {
            return
        }
        if (user == null) {
            return
        ...
        if (user.player.hasPermission("funnyguilds.admin.disabledummy")) {
            return
        }
        val scoreboard = this.user.cache.scoreboard
        if (scoreboard == null) {
            FunnyGuilds.Companion.getPluginLogger().debug(
                "We're trying to update Dummy score but scoreboard hasn't been initialized yet" +
                        "(maybe player left the game while updating?)"
            )
            return
        }
        val objective: Objective = scoreboard.getObjective(Dummy.Companion.OBJECTIVE_NAME)
        if (objective == null || objective.name != Dummy.Companion.OBJECTIVE_NAME) {
            initialize()
        } else {
            objective.getScore(user.name!!).score = user.rank.points
        }
    }
    ...
    private fun initialize() {
        ...
        val scoreboard = user.cache.scoreboard
        if (scoreboard == null) {
            FunnyGuilds.Companion.getPluginLogger().debug(
        ...
        var objective: Objective = scoreboard.getObjective(Dummy.Companion.OBJECTIVE_NAME)
        if (objective == null || objective.name != Dummy.Companion.OBJECTIVE_NAME) {
            objective = scoreboard.registerNewObjective(Dummy.Companion.OBJECTIVE_NAME, "dummy")
            objective.displaySlot = DisplaySlot.BELOW_NAME
            objective.displayName = FunnyGuilds.Companion.getInstance().getPluginConfiguration().dummySuffix
        }
    ...
    companion object {
        private const val OBJECTIVE_NAME = "FG-Points"
    }
