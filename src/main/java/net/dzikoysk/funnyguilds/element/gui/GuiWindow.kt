package net.dzikoysk.funnyguilds.element.guiimport
...
class GuiWindow {
    ...
    private val items: MutableMap<Int, GuiItem>
    private var openHandler: Consumer<InventoryOpenEvent>? = null
    private var closeHandler: Consumer<InventoryCloseEvent>? = null

    constructor(name: String, rows: Int) {
        this.name = getValidName(name)
        inv = Bukkit.createInventory(null, if (rows > 6) 6 * 9 else rows * 9, this.name)
        items = HashMap(if (rows > 6) 6 * 9 else rows * 9)
        GuiWindow.Companion.windows.put(this.name, this)
    }

    constructor(name: String, items: List<ItemStack?>) {
        this.name = getValidName(name)
        inv = Bukkit.createInventory(null, roundUp(items.size), this.name)
        this.items = HashMap(roundUp(items.size))
        GuiWindow.Companion.windows.put(name, this)
    }

    fun setItem(slot: Int, item: GuiItem) {
        items[slot] = item
        inv.setItem(slot, item.wrap())
    }
    ...
    fun setToNextFree(item: GuiItem) {
        for (i in 0 until inv.size) {
            if (inv.getItem(i) == null) {
                items[i] = item
                inv.setItem(i, item.wrap())
                break
            }
        }
    }

    fun setToNextFree(item: GuiItem, start: Int) {
        for (i in start until inv.size) {
            if (inv.getItem(i) == null) {
                items[i] = item
                inv.setItem(i, item.wrap())
                break
    ...
    fun setOpenEvent(e: Consumer<InventoryOpenEvent>?) {
        openHandler = e
    }

    fun setCloseEvent(e: Consumer<InventoryCloseEvent>?) {
        closeHandler = e
    }

    fun handleOpen(e: InventoryOpenEvent) {
        if (openHandler != null) {
            openHandler!!.accept(e)
        }
    }
    ...
    fun open(entity: HumanEntity) {
        val inv = Bukkit.createInventory(entity, wrap().size, name)
        inv.contents = wrap().contents
        entity.openInventory(inv)
    }

    fun unregister() {
        GuiWindow.Companion.windows.remove(name)
        items.clear()
    }

    private fun getValidName(name: String): String {
        return if (GuiWindow.Companion.windows.containsKey(name)) {
            getValidName(name + ChatColor.RESET)
        } else {
            name
        }
    }

    fun fillEmpty(itemStack: ItemStack) {
    ...
    fun fillEmpty(material: Material) {
        ...
        for (i in 0 until inv.size) {
            if (inv.getItem(i) == null) {
                inv.setItem(i, ItemStack(material, 1))
            }
        }
    }
    ...
    companion object {
        private val windows: Map<String, GuiWindow> = HashMap()
        fun getWindow(inv: String?): GuiWindow {
            return GuiWindow.Companion.windows.get(inv)
        }
    }
}